...
{
	char buffl[CHARL];*buffl='\0';
	char buffn[CHARN];*buffn='\0';
	char buffo[CHARN];*buffo='\0';
	char buff_stoch[CHARN];*buff_stoch='\0';
	char numerical[CHARN];*numerical='\0';
	char line_clear[CHARL];*line_clear='\0';
	bool ident=false;
	...
	if (getcwd(current_dir, sizeof(current_dir)) == NULL) {
		cerr << "*** Error: Could not get current working directory ***\n";
		exit(1);
	}
	string input_dir = string(current_dir) + "/../data";
	cout << "Current working directory: " << input_dir << endl;
	string file_path = input_dir + "/" + "input.asc";
	string file_path_copy = input_dir + "/" + "input_copy.asc";

	fstream input1(file_path);
	if(!input1){cout<<" *** Error: cannot open 'input.asc' file *** \n";exit(1);}

	//open new copy file
	fstream fcopy(file_path_copy);
	...
	ofstream input;
	input.open(file_path,ios::out|ios::trunc);
