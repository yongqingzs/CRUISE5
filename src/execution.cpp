...
int main(void) 
{
	...
	if(input.fail()) {
		cerr << "*** Error: File stream '" << input_path << "' failed to open (check spelling) ***\n";
		exit(1);
	}

	ofstream ftabout(tabout_path.c_str());
	if(!ftabout) {
		cout << " *** Error: cannot open '" << tabout_path << "' file *** \n";
		exit(1);
	}

	ofstream fdoc(doc_path.c_str());
	if(!fdoc) {
		cout << " *** Error: cannot open '" << doc_path << "' file *** \n";
		exit(1);
	}

	ofstream ftraj(traj_path.c_str());
	if(!ftraj) {
		cout << " *** Error: cannot open '" << traj_path << "' file *** \n";
		exit(1);
	}
